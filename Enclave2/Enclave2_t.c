...
typedef struct ms_generate_random_number2_t {
	int ms_retval;
} ms_generate_random_number2_t;

typedef struct ms_seal2_t {
...
static sgx_status_t SGX_CDECL sgx_generate_random_number2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_generate_random_number2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_generate_random_number2_t* ms = SGX_CAST(ms_generate_random_number2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = generate_random_number2();


	return status;
}

static sgx_status_t SGX_CDECL sgx_seal2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_seal2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_seal2_t* ms = SGX_CAST(ms_seal2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ms->ms_retval = seal2(_in_plaintext, _tmp_plaintext_len, _in_sealed_data, _tmp_sealed_size);
	if (_in_sealed_data) {
...
static sgx_status_t SGX_CDECL sgx_unseal2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_unseal2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_unseal2_t* ms = SGX_CAST(ms_unseal2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	sgx_sealed_data_t* _tmp_sealed_data = ms->ms_sealed_data;
	size_t _tmp_sealed_size = ms->ms_sealed_size;
	size_t _len_sealed_data = _tmp_sealed_size;
	...
	ms->ms_retval = unseal2(_in_sealed_data, _tmp_sealed_size, _in_plaintext, _tmp_plaintext_len);
	if (_in_plaintext) {
...
SGX_EXTERNC const struct {
	...
	3,
	{
		{(void*)(uintptr_t)sgx_generate_random_number2, 0},
		{(void*)(uintptr_t)sgx_seal2, 0},
		{(void*)(uintptr_t)sgx_unseal2, 0},
	}
};
